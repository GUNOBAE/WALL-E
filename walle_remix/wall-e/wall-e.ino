...
#define BAT_L
	...
	#define BATTERY_MAX_VOLTAGE 12.0
	#define BATTERY_MIN_VOLTAGE 10.0
	#define DIVIDER_SCALING_FACTOR 0.3197
  #define BUZZER_PIN 4                    // 부저 핀 정의
  const float LOW_BATTERY_THRESHOLD = 5.0;  // 부저 경고 기준 퍼센트
  const float CALIBRATION_FACTOR = 11.9 / 12.8; // 소프트웨어 보정 계수
...
int preset[][2] =  {{420,150},  // head rotation
                    {655,130},  // neck top 
                    {150,630},  // neck bottom
                    {135,310},  // eye right 
                    {330,140},  // eye left
...
void setup() {
	...
	// If an oLED is present, start it up
		...
		u8g2.begin();
		displayLevel(100, BATTERY_MAX_VOLTAGE);
	#endif
...
void checkBatteryLevel() {
  ...
  // 1. 아날로그 핀 값을 읽어 전압 계산
  float voltage = analogRead(BATTERY_LEVEL_PIN) * 5.0 / 1023.0;
  voltage = voltage / DIVIDER_SCALING_FACTOR;

  // 2. 보정 계수를 적용하여 최종 전압 계산
  voltage = voltage * CALIBRATION_FACTOR;
  
  // 3. 전압을 기준으로 배터리 잔량(%) 계산
  int percentage = 0;
  if (voltage >= BATTERY_MAX_VOLTAGE) {
    ...
    percentage = (int)(((voltage - BATTERY_MIN_VOLTAGE) / (BATTERY_MAX_VOLTAGE - BATTERY_MIN_VOLTAGE)) * 100.0);
  }

 // 4. OLED 디스플레이에 전압과 퍼센트 표시
  #ifdef OLED
    // 업그레이드된 함수에 전압과 퍼센트를 모두 전달
    displayLevel(percentage, voltage);
  #endif
